...
- **RULE-SVC-01** — At quote approval, seed every step template that passes all four gates — **package → CRO mode → LC mode → service**, each *empty means don't gate* — in canonical order (ADR-040/046/050).
...
- **RULE-SVC-04** — `lc_finance` is implied when the customer's `source` is `bank_lc` **or** when `lc_handled_by = consort` (ADR-050), and may also be selected explicitly (ADR-041/042). It is additive on top of any package. One carve-out: a bank-LC customer who explicitly chooses `customer` LC handling suppresses the injection — we are not selling LC legwork on a trade they run themselves.
...
- **RULE-SVC-10** — **The LC mode follows the same triple enforcement as the CRO mode** (ADR-050): `allowedLcModes` (only the export packages may hold `customer`/`consort`; `not_applicable` = open-account trade stays selectable there), the intake schema's coherence refine, and the `*_lc_mode_valid` CHECK constraints. `resolveLcMode` normalises the column — bank-LC customers default to `consort`; a row whose frozen `services` carry `lc_finance` reads `consort` — but **composition never depends on the coercion**: the Consort LC steps (30/130) compose off their `lc_finance` service gate exactly as before ADR-050; only `lc_received_from_customer` (35) is lc-modes-gated.
- **RULE-SVC-11** — **The step/document catalog is admin-managed data** (ADR-051, Workflow panel, `workflow.manage`, Management-only). Edits shape **new shipments only** (INV-14). A step code is immutable; a step or document type referenced by any shipment, charge type or template is **deactivated, never deleted**. Every step write re-derives its `otd.step` TaskTemplate in the same transaction (`utils/taskTemplates.js`), and every mutation is audited. `node prisma/seed.js --templates-only` requires `--factory-reset` because it replaces the curated catalog with factory rows.
