...
const ShipmentCard = ({ shipment: s, onDocuments }) => {
  ...
  // Those two steps only exist on a shipment that is waiting for the customer's copy —
  // the handling modes that used to qualify this are gone, so the pending step IS the ask.
  const awaitingCro = (s.otdSteps ?? []).some(
    (st) => st.stepCode === "cro_received_from_customer" && st.status !== "done",
  );
  const awaitingLc = (s.otdSteps ?? []).some(
    (st) => st.stepCode === "lc_received_from_customer" && st.status !== "done",
  );
 ...
 *
 * A query is a plain enquiry now — name, email, phone, the two addresses and the
 * services wanted — so this is the shared QueryFormModal with the customer locked:
 * a portal user is always scoped to their own customer, which the API fills in.
 */
const NewQueryDialog = ({ onClose, onCreated }) => {
  ...
  const submit = async (payload) => {
    ...
    try {
      ...
      toast.success("Request sent — we will come back with a quote");
      onCreated?.();
      onClose();
    } catch (err) {
  ...
  return <QueryFormModal mode="create" lockCustomer busy={busy} onClose={onClose} onSubmit={submit} />;
};
